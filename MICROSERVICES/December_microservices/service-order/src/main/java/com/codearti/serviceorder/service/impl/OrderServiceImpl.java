package com.codearti.serviceorder.service.impl;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Mono<OrderResponse> updateOrder(int orderId) {
        return orderRepository.findById(orderId)
                ...
                .doOnSuccess(p -> {
                    ...
                    this.sendMessageKafka(p.getId() + ":" + p.getStatus().getValue()).subscribe();
                })
    ...
    private Mono<Void> sendMessageKafka(String valueNotification) {
        log.info("Sending message to Kafka topic: {}", valueNotification);
        return Mono.fromCallable(() ->kafkaTemplate.send("order-topic", valueNotification)).then();
    }
